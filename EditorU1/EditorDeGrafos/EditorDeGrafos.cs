...
namespace EditorDeGrafos
{
    public partial class EditorDeGrafos : Form
    {
        ...
        private void NoDirigido_Clicked(object sender, ToolStripItemClickedEventArgs e)
        {
            ...
            switch (e.ClickedItem.AccessibleName)
            {
                ...
                case "Isomorfismo":
                    ...
                    Grafo grafito; //me segundo grafo
                    GrafoSecundario sGrafo; // form para dibujar grafo
                    List<int[,]> pasos; //lista de pasos matriz
                    List<int> cambiados; //cambiados
                    int[] cambios;   //cambios
                    bool band;   //bandera de isomorfismo
                    int[,] matrizCambio;   //matriz final de cambio
                    sGrafo = new GrafoSecundario(grafo);
                    pasos = new List<int[,]>();  //lista de matricies
                    ...
                    if (sGrafo.ShowDialog() == DialogResult.OK)
                    {
                        ...
                        if (grafo.Count == grafito.Count)
                        {
                            ...
                            {
                                pasos.Add(grafito.matrizDeAdyacencia()); // se agrega en la lista los pasos
                                band = grafo.isomorfismo(grafo.matrizDeAdyacencia(), grafito.matrizDeAdyacencia(), ref pasos, ref cambios); // si todo sale bien es true
                                matrizCambio = grafito.matrizDeAdyacencia(); // se igualan las matrixces para imprimir
                                if (band)
                                {
        ...
        {
            // Propiedades pro = new Propiedades(grafo,grafo.Tipo);
            // pro.ShowDialog();

        }

        private void matrizDeAdyasenciaToolStripMenuItem_Click(object sender, EventArgs e)
        {
            MatrizDeAdyacencia matrizA;
            matrizA = new MatrizDeAdyacencia(grafo.grafoMatriz());
            matrizA.ShowDialog();
        ...
        private void gradoGrafoToolStripMenuItem_Click(object sender, EventArgs e)
        {
        ...
        private void isomorfismoToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            List<int[,]> pasos2;
            List<int> cambiados;
            int[] cambios; // arreglo de enteros
            bool band;
            int[,] matrizCambio;
            sGrafo = new GrafoSecundario(grafo);
            pasos = new List<int[,]>(); //lista de arreglos dobles o matrices
            pasos2 = new List<int[,]>(); //lista de arreglos dobles o matrices
            cambios = new int[grafo.Count];
            cambiados = new List<int>(); //lista de enteros
            ...
            if (sGrafo.ShowDialog() == DialogResult.OK)
            {
                ...
                if (grafo.Count == grafito.Count)
                {
                    if (grafo.Aristas == grafito.Aristas)
                    {
                            ...
                            foreach (var elemento in pasos2)
                            {
                                if (pasos != null)
                                {
                                    for (int i = 0; i < grafo.Count; i++)
                                    {
                                        ...
                                        {
                                            escribir.Write(elemento[i,j]);
                                            // MessageBox.Show(elemento[i,j].ToString());
                        ...
                        for (int i = 0; i < grafito.Count; i++) //UNA VEZ COMPROBADO SE MANDAN A IMPRIMIR
                        {
                            if (!MetodosAuxiliares.seAhCambio(i, cambiados) && !MetodosAuxiliares.seAhCambio(cambios[i], cambiados))
                            {
                                matrizCambio = MetodosAuxiliares.cambioIsomorfico(matrizCambio, i, cambios[i]);
                                cambiados.Add(i);
                               // MessageBox.Show("con i " + cambios[i].ToString());
                                cambiados.Add(cambios[i]);
                                //MessageBox.Show("con camnios[i] " + cambios[i].ToString());
                            }
                        }
                        isomorfismo = new Isomorfismo(grafo, grafito, pasos, cambios, band, matrizCambio);
                        isomorfismo.ShowDialog();
        ...
        private void ciclicoToolStripMenuItem_Click(object sender, EventArgs e)
        {
